...
Since the validation rules are creating dynmically they need to be included in the header of your layout file:-
  ...
  <%= javascript_include_tag 'green-light', 'data-turbolinks-track' => true %>

Point the <tt>data-validatable</tt> to <tt>green-light</tt> in the forms that you wish to have client side validation:-

  <%= form_for(@model, :html => { 'data-validatable' => 'green-light' }) do |f| %>
